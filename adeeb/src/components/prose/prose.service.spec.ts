...
describe.concurrent('Testing ProseService', async () => {
  ...
  describe("Testing post()", async() => {
    ...
    test("it returns false after inValid validation", async() => {
      ...
      const result1 = await ProseService.post({tags, qoute} as unknown as Prose);
      expect(result1).toStrictEqual(false);
      const result2 = await ProseService.post({tags, poet} as unknown as Prose);
      expect(result2).toStrictEqual(false);
      const result3 = await ProseService.post({ qoute, poet} as unknown as Prose);
