...
export const PoemController = {
  indexWithPoetName: async (req: Request, res: Response, next: NextFunction) => {
    try {
  ...
  indexIntrosWithPoetName: async (req: Request, res: Response, next: NextFunction) => {
    try {
      const service = await PoemService.getAllIntrosWithPoetName();
      const {status, poems, errMsg} = responseInfo.indexIntrosWithPoetName(service)
      if (errMsg)
        throw new AppError(status, errMsg,true);
...
export const responseInfo = {
  ...
  postMany: (poems: {newPoems: Poem[], inValidPoems: Poem[]} | false): {status: number, poems?: {newPoems: Poem[], inValidPoems: Poem[]}, errMsg?: string} => {
    if (!poems) {
      return {status: HttpStatusCode.NOT_ACCEPTABLE, errMsg: ERROR_MSG.NOT_VALID }
    }
