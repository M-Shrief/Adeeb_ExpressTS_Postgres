...
describe.concurrent("Testing PoemService", async() => {
  ...
  describe("Testing getOneWithPoet()", async() => {
    ...
    it("Returns false if no data is not found", async() => {
      ...
      const result = await PoemService.getOneWithPoet( "e7749f21-9cf9-4981-b7a8-2ce262f159f6")
      expect(result).toEqual(false)
    })
  })
  ...
  describe("Testing post()", async() => {
    ...
    test("Return false if data validation failed", async() => {
      ...
      const result1 = await PoemService.post({intro, poet} as unknown as Poem)
      expect(result1).toEqual(false)
      const result2 = await PoemService.post({intro, verses} as unknown as Poem)
      expect(result2).toEqual(false)
      const result3 = await PoemService.post({poet, verses} as unknown as Poem)
  ...
  describe("Testing postMany()", async() => {
    ...
    test("Post valid data successfully after validation and return inValid ones", async() => {
      ...
      const result= await PoemService.postMany([poem1,poem2,poem3,poem4,poem5, poem6]);
  ...
  describe("Testing update()", async() => {
    ...
    test("Update poem data successfully after validation", async() => {
      vi.spyOn(PoemDB, "update").mockResolvedValue({affected: 1} as UpdateResult);

      const result1 = await PoemService.update("1",{intro} as Poem)
      expect(result1).toEqual(1)
      const result2 = await PoemService.update("1",{poet} as unknown as Poem)
    ...
    test("return false after invalid poemData",async () => {
      ...
      expect(result1).toEqual(false)
      const result2 = await PoemService.update("1",{poet: '214'} as unknown as Poem)
      expect(result2).toEqual(false)
      const result3 = await PoemService.update("1",{verses: [{first: 'as', sec: "a"}]} as Poem)
      expect(result3).toEqual(false)
    })
    test("return false after non-existing id",async () => {
      vi.spyOn(PoemDB, "update").mockResolvedValue({affected: 0} as UpdateResult);      
  ...
  describe("Testing remove()", async() => {
    test("Successfully deletes poem", async() => {
      vi.spyOn(PoemDB, "remove").mockResolvedValue({affected: 1} as DeleteResult);      

      const result1 = await PoemService.remove("1")
      expect(result1).toEqual(1)
    })
    test("return false for non-existing id", async() => {
      vi.spyOn(PoemDB, "remove").mockResolvedValue({affected: 0} as DeleteResult);      
